(()=>{
  const pairs=[
    {id:'featuresHi',title:'Key Features — Hindi',hint:'एक feature के लिए एक bullet रखें',placeholder:'मुख्य विशेषता लिखें...'},
  ...
  const state={};
  const syncTextarea=id=>{const ta=document.getElementById(id);if(ta)ta.value=(state[id]||[]).filter(Boolean).join('\n')};
  ...
  function setFromTextarea(id,ensureBlank=true){
    state[id]=read(id).map(x=>x.trim()).filter(Boolean);
    if(ensureBlank&&!state[id].length)state[id]=[''];
    render(id,ensureBlank?Math.max(0,state[id].length-1):-1)
  }
  function build(pair){
    ...
    box.querySelector('.ve-feature-list').addEventListener('click',e=>{const b=e.target.closest('.ve-feature-remove');if(!b)return;e.preventDefault();const i=Number(b.dataset.index);state[pair.id].splice(i,1);if(!state[pair.id].length)state[pair.id]=[''];render(pair.id,Math.min(i,state[pair.id].length-1))});
    box.querySelector('.ve-feature-list').addEventListener('input',e=>{const input=e.target.closest('.ve-feature-input');if(!input)return;const i=Number(input.dataset.index);state[pair.id][i]=input.value;syncTextarea(pair.id)});
    box.querySelector('.ve-feature-list').addEventListener('keydown',e=>{const input=e.target.closest('.ve-feature-input');if(!input||e.key!=='Enter')return;e.preventDefault();const i=Number(input.dataset.index);state[pair.id][i]=input.value;state[pair.id].splice(i+1,0,'');render(pair.id,i+1)});
    box.querySelector('.ve-feature-list').addEventListener('paste',e=>{const input=e.target.closest('.ve-feature-input');if(!input)return;const text=e.clipboardData?.getData('text')||'';if(!text.includes('\n'))return;e.preventDefault();const i=Number(input.dataset.index);const parts=text.split(/\r?\n/).map(strip).map(x=>x.trim()).filter(Boolean);state[pair.id].splice(i,1,...(parts.length?parts:['']));render(pair.id,i)});
    syncTextarea(pair.id);render(pair.id,state[pair.id].length-1)
  }
  function hookEditor(){
    if(typeof window.openEditor!=='function'||window.__veFeatureHooked)return;
    const original=window.openEditor;window.openEditor=function(p=null){original(p);pairs.forEach(x=>setFromTextarea(x.id,true))};window.__veFeatureHooked=true
  }
