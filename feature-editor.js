(()=>{
  ...
  function getItems(id){
    if(Array.isArray(state[id]))return state[id].slice();
    const items=clean(textarea(id)?.value).filter(Boolean);
    state[id]=items.slice();
    return items;
  }
  function saveItems(id,items){
    const normalized=items.map(x=>String(x??'').trim());
    state[id]=normalized.slice();
    const ta=textarea(id);
    if(ta){
      ta.value=normalized.filter(Boolean).join('\n');
      ta.dispatchEvent(new Event('input',{bubbles:true}));
      ta.dispatchEvent(new Event('change',{bubbles:true}));
    }
  }
  function render(id,focusIndex=-1){
    const ta=textarea(id),box=document.getElementById(id+'Editor');
    if(!ta||!box)return;
    const items=getItems(id);
    const list=box.querySelector('.ve-feature-list');
    list.innerHTML=items.length
      ? items.map((item,i)=>`<div class="ve-feature-row"><span class="ve-feature-bullet" aria-hidden="true">•</span><input class="ve-feature-input" data-index="${i}" type="text" value="${esc(item)}" placeholder="${id==='featuresHi'?'मुख्य विशेषता लिखें...':'Enter a key feature...'}"><button type="button" class="ve-feature-remove" data-index="${i}" aria-label="Remove feature">×</button></div>`).join('')
      : '<div class="ve-feature-empty">No bullet points yet. Click <strong>＋ Add feature</strong> to start.</div>';
    if(focusIndex>=0){
      const input=list.querySelector(`.ve-feature-input[data-index="${focusIndex}"]`);
      if(input){input.focus();input.setSelectionRange(input.value.length,input.value.length)}
    }
  }
  function build(pair){
    const ta=textarea(pair.id);if(!ta||document.getElementById(pair.id+'Editor'))return;
    const label=ta.closest('label');if(!label)return;
    state[pair.id]=clean(ta.value).filter(Boolean);
    ta.style.display='none';
    const box=document.createElement('div');
    box.id=pair.id+'Editor';
    box.className='ve-feature-editor';
    ...
    box.addEventListener('click',e=>{
      ...
      if(add){
        ...
        const next=getItems(pair.id);next.push('');
        saveItems(pair.id,next);
      ...
      if(remove){
        e.preventDefault();e.stopPropagation();
        const i=Number(remove.dataset.index),next=getItems(pair.id);
        if(Number.isInteger(i))next.splice(i,1);
        saveItems(pair.id,next);render(pair.id);return;
      }
    });

    box.addEventListener('input',e=>{
      const input=e.target.closest('.ve-feature-input');if(!input)return;
      const i=Number(input.dataset.index),next=getItems(pair.id);
      if(Number.isInteger(i)){next[i]=input.value;saveItems(pair.id,next);}
    });
    ...
    box.addEventListener('keydown',e=>{
      ...
      next[i]=input.value;next.splice(i+1,0,'');saveItems(pair.id,next);render(pair.id,i+1);
    });
    ...
    box.addEventListener('paste',e=>{
      ...
      e.preventDefault();
      const i=Number(input.dataset.index),pasted=clean(text).filter(Boolean),next=getItems(pair.id);
      next.splice(i,1,...(pasted.length?pasted:['']));
      saveItems(pair.id,next);render(pair.id,i);
    });
